...
class HandlerTest {
    ...
    void testHandleRequest_success() throws Exception {
        UpdateTransactionRequest request = new UpdateTransactionRequest("12345", "acc-num-123", TransactionUtil.Status.SUCCESS);
        Map<String, Object> input = objectMapper.convertValue(request, Map.class);
        PowerMockito.doNothing().when(handler, "updateTransaction");
    ...
    void testHandleRequest_failure() throws Exception{
        ...
        PowerMockito.doThrow(new RuntimeException("Database error")).when(handler, "updateTransaction");
